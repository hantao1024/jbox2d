package com.blofin.myjbox2d.two;
...
//        }
    ...
    private void createWorld(boolean haveDifferent, Context context) {
        if (world == null) {
            world = new World(new Vec2(0f, 2f));//创建世界,设置重力方向
        }
        if (world != null) {
            // 初始化圆形世界边界
            initCircleWorldBounds(canvas);
...
//        }
        ...
        fixtureDef.density = density;//物质密度
        fixtureDef.friction = friction;//摩擦系数
        fixtureDef.restitution =restitution;//恢复系数
        ...
        fixtureDef2.density = density;//物质密度
        fixtureDef2.friction = friction;//摩擦系数
        fixtureDef2.restitution = restitution;//恢复系数
...
//        bodyDef.position.set(0, -pixelsToMeters(pHeight/10*9));
//        bodyDef.position.set(0, -pixelsToMeters(pWidth/2));
//        bodyDef.position.set(0, -pixelsToMeters(pWidth/2/2));
//        bodyDef.position.set(-pixelsToMeters(pHeight/2), -pixelsToMeters(pWidth/2/2));
//        bodyDef.position.set(pixelsToMeters(pWidth/2), -pixelsToMeters(pHeight/10*9));
        Body bodyTop = world.createBody(bodyDef);//世界中创建刚体
        FixtureDef fixtureDef = new FixtureDef();
        fixtureDef.density = density;//物质密度
        fixtureDef.friction = friction;//摩擦系数
        ...
        //设置圆形刚体边界
        ArrayList positions = polygon2(5, Double.valueOf(pHeight/2).intValue());
//        ArrayList positions = polygon2(360, Double.valueOf(pHeight ).intValue());
